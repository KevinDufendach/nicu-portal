...
/**
 * Inject all the files into the new index2.html
 * rev, but no map
 * @return {Stream}
 */
gulp.task('rev-and-inject', ['js', 'vendorjs', 'css', 'vendorcss'], function() {
    log('Rev\'ing files and building index2.html');
    ...
    var stream = gulp
        ...
        .src([].concat(minified, index)) // add all built min files and index2.html
        .pipe(minFilter) // filter the stream to minified css and js
        .pipe(plug.rev()) // create files with rev's
        .pipe(gulp.dest(paths.build)) // write the rev files
        .pipe(minFilter.restore()) // remove filter, back to original stream

    // inject the files into index2.html
    .pipe(indexFilter) // filter to index2.html
    ...
    // replace the files referenced in index2.html with the rev'd files
    .pipe(plug.revReplace()) // Substitute in new filenames
    .pipe(gulp.dest(paths.build)) // write the index2.html file changes
